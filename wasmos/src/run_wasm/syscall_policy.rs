...
mod tests {
    ...
    fn permissive_allows_everything() {
        let p = SyscallPolicy::permissive();
        assert_eq!(p.check("host_log"), PolicyAction::Allow);
        assert_eq!(p.check("arbitrary_call"), PolicyAction::Allow);
        assert_eq!(p.check("dangerous"), PolicyAction::Allow);
    }

    #[test]
    fn strict_denies_unlisted_imports() {
        let mut p = SyscallPolicy::strict();
        assert_eq!(p.check("host_log"), PolicyAction::Deny);

        // Explicitly allow one call — it should now pass.
        p.allowed.push("host_log".to_string());
        assert_eq!(p.check("host_log"), PolicyAction::Allow);
        assert_eq!(p.check("other_call"), PolicyAction::Deny);
    }

    #[test]
    fn sandbox_allows_standard_abi_and_blocks_others() {
        let p = SyscallPolicy::sandbox();
        assert_eq!(p.check("host_log"), PolicyAction::Allow);
        assert_eq!(p.check("read_sensor"), PolicyAction::Allow);
        assert_eq!(p.check("send_alert"), PolicyAction::Allow);
        assert_eq!(p.check("dangerous_call"), PolicyAction::Deny);
    }
    ...
    fn block_list_beats_allow_list() {
        let p = SyscallPolicy {
            preset: PolicyPreset::Permissive,
            allowed: vec!["dangerous".to_string()],
            blocked: vec!["dangerous".to_string()],
            label: "test".to_string(),
            default_action: PolicyAction::Allow,
            max_violations: 10,
            log_allowed: false,
        };
        // blocked wins over allowed for the same name.
        assert_eq!(p.check("dangerous"), PolicyAction::Deny);
    }

    #[test]
    fn policy_request_into_policy_applies_preset_and_overrides() {
        let req = PolicyRequest {
            preset: PolicyPreset::Strict,
            allowed: vec!["host_log".to_string()],
            blocked: vec![],
        };
        let policy = req.into_policy();
        assert_eq!(policy.check("host_log"), PolicyAction::Allow);
        assert_eq!(policy.check("other"), PolicyAction::Deny);
    }

    #[test]
    fn syscall_violation_new_round_trips() {
        let v = SyscallViolation::new(
            "dangerous",
            "env",
            42,
            "blocked by strict policy".to_string(),
        );
        assert_eq!(v.name, "dangerous");
        assert_eq!(v.module, "env");
        assert_eq!(v.instruction_index, 42);
        assert!(v.reason.contains("strict"));
    }

    #[test]
    fn label_method_returns_string_slice() {
        let p = SyscallPolicy::permissive();
        assert_eq!(p.label(), "permissive");

        let p = SyscallPolicy::strict();
        assert_eq!(p.label(), "strict");
    }
}